...
const AddContact = ({ contact, deleteContact, addContact }) => {
  ...
  <>
    <div className='container text-center '>
      <div>
        <h3>Contacts</h3>
        <form className='row justify-content-between' onSubmit={handleSubmit1}>
          <input
            type='text'
            placeholder='name'
          ...
          <thead>
           ...
           {contact.map((contact, id) => (
              <tr key={id}>
                ...
                <td >{contact.email}</td>
                <td>
                  <button className='btn btn-danger ' onClick={() => deleteContact(contact.id)} >Remove</button>
                </td>
              </tr>
           ))}
          </thead>
...
AddContact.propTypes = {
  contact: propTypes.any, deleteContact: propTypes.any, addContact: propTypes.any
}
